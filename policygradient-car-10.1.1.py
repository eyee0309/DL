...
# Recall: softplus(x) = log(1 + exp(x)), essentially a softened
# version of the restricted linear unit (ReLU) where ReLU(x) = max(0, x)
def softplusk(x):
...
class PolicyAgent():
    ...
    # remember every s,a,r,s' in every step of the episode (trajectory)
    ...
    # the log probability of the Gaussian distribution (policy)
    def logp(self, args):
    ...
    # entropy of the policy H(\pi(a|s))
    ...
    def build_autoencoder(self):
        ...
        # Input - Dense(256) - Dense(128) - Dense(32) (first 2 layers
        ...
        # Output of encoder - Dense(128) - Dense (256) - Dense(state_dim = 2)
        # first 2 layers have ReLU activation
    ...
    def build_actor_critic(self):
        inputs = Input(shape=(self.state_dim, ), name='state')
        self.encoder.trainable = False   # trained encoder, so parameters are frozen
        x = self.encoder(inputs)
        # Policy model (actor model) \pi(a|s, \theta)
        #============================
        mean = Dense(1,
                     activation='linear',
        ...
        # log(\pi(a|s, \theta)) = Gaussian log probability
        ...
        # Entropy of policy: H(\pi(a|s,\theta))
        ...
        # beta (weight) of entropy used in A2C only for loss function
        beta = 0.9 if self.args.a2c else 0.0

        # logp (log(\pi(a|s, \theta)) loss of policy network
        ...
        # loss function called value loss (viz., loss for value network)
    ...
    # return the entropy of the policy distribution \pi(a|s, \theta)
    ...
    def train_by_episode(self, last_value=0):
        ...
        for i in range(len(rewards)):
            ...
            self.memory[i][3] = return_     # return replaced reward in memory
    ...
    def train(self, item, gamma=1.0):
        ...
        discounted_delta = delta * discount_factor   # \gamma^t * \delta
    ...
    for episode in range(episode_count):
        ...
        while not done:
            ...
            if args.actor_critic and train:
                ...
                # train at every step (of episode) as it happens
